...
DROP TABLE IF EXISTS relay_channels CASCADE;
DROP TABLE IF EXISTS vending_machines CASCADE;
DROP TABLE IF EXISTS relay_devices CASCADE;
DROP TABLE IF EXISTS water_pumps CASCADE;
...
DROP POLICY IF EXISTS "Users can view their own vending machines" ON vending_machines;
...
DROP POLICY IF EXISTS "Users can update their own vending machines" ON vending_machines;
CREATE POLICY "Users can update their own vending machines" ON vending_machines
  FOR UPDATE USING (auth.uid() = owner);

DROP POLICY IF EXISTS "Admins can create vending machines" ON vending_machines;
...
DROP POLICY IF EXISTS "Users can view their own relay devices" ON relay_devices;
CREATE POLICY "Users can view their own relay devices" ON relay_devices
  FOR SELECT USING (auth.uid() = owner OR owner IS NULL);

DROP POLICY IF EXISTS "Users can update their own relay devices" ON relay_devices;
CREATE POLICY "Users can update their own relay devices" ON relay_devices
  FOR UPDATE USING (auth.uid() = owner);

DROP POLICY IF EXISTS "Admins can create relay devices" ON relay_devices;
...
DROP POLICY IF EXISTS "Users can view their own water pumps" ON water_pumps;
CREATE POLICY "Users can view their own water pumps" ON water_pumps
  FOR SELECT USING (auth.uid() = owner OR owner IS NULL);

DROP POLICY IF EXISTS "Users can update their own water pumps" ON water_pumps;
CREATE POLICY "Users can update their own water pumps" ON water_pumps
  FOR UPDATE USING (auth.uid() = owner);

DROP POLICY IF EXISTS "Admins can create water pumps" ON water_pumps;
...
RETURNS TRIGGER AS $$
BEGIN
  -- Create 4 output channels (most common for relay devices)
  FOR i IN 1..4 LOOP
    INSERT INTO relay_channels (relay_device_id, channel_number, channel_type, display_name)
    VALUES (NEW.id, i, 'output', 'Channel ' || i);
  END LOOP;
...
DROP TRIGGER IF EXISTS update_relay_devices_updated_at ON relay_devices;
CREATE TRIGGER update_relay_devices_updated_at BEFORE UPDATE ON relay_devices
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_water_pumps_updated_at ON water_pumps;
